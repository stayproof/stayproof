...
if (typeof importScripts === 'function') {
  ...
  importScripts('../shared/currency.js');
}
...
/**
 * Build a Booking.com search URL with the given currency and English locale.
 * Pure function — no side effects.
 */
function buildBookingSearchUrl(destination, checkin, checkout, maxPrice, currency) {
  ...
  if (maxPrice && isFinite(maxPrice)) {
    params.set('nflt', 'price=' + cur + '-min-' + maxPrice + '-1');
  }
  return 'https://www.booking.com/searchresults.en-us.html?' + params.toString();
}

/**
 * Build an Airbnb search URL with the given currency and English locale.
 * Pure function — no side effects.
 */
/**
...
/**
 * Build an Agoda search URL with the given currency and English locale.
 * Requires a numeric city ID (resolved via resolveAgodaCityId).
 */
function buildAgodaSearchUrl(cityId, areaId, destination, checkin, checkout, maxPrice, currency) {
  var cur = currency || 'USD';
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'startSearch') {
    ...
    var currency = message.currency || null;
    ...
    var bookingUrl = buildBookingSearchUrl(dest, checkin, checkout, maxPrice, currency);
    var airbnbOpts = {
    ...
    openSearchTab(bookingUrl, function(bookingTab) {
      ...
      resolveAgodaCityId(dest).then(function(result) {
        ...
        var agodaUrl = buildAgodaSearchUrl(result.cityId, result.areaId, result.name, checkin, checkout, maxPrice, currency);
